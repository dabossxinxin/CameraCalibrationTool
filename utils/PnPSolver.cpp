...
{
    void PnPSolver::SolvePnP(Eigen::Matrix3d& R, Eigen::Vector3d& t)
    {
        cv::FileStorage fs(mStrSettingFile.c_str(), cv::FileStorage::READ);
        ...
        solvePnPbyDLT(K, mPts3d, mPts2d, R, t);
        //solvePnPbyEPnP(K, mPts3d, mPts2d, R, t);
    };


    bool PnPSolver::solvePnPbyP3P(const Eigen::Matrix3d& K, const std::vector<Eigen::Vector3d>& pts3d, const std::vector<Eigen::Vector2d>& pts2d, Eigen::Matrix3d& R, Eigen::Vector3d& t)
    {
        // Check Input Data
        if (pts3d.size() != pts2d.size() || pts3d.size() != 3)
        {
            return false;
        }
        ...
        // transform
        Eigen::Vector3d pc0((pts2d[0](0) - cx) * inv_fx, (pts2d[0](1) - cy) * inv_fy, 1); pc0 /= pc0.norm();
        Eigen::Vector3d pc1((pts2d[1](0) - cx) * inv_fx, (pts2d[1](1) - cy) * inv_fy, 1); pc1 /= pc1.norm();
        Eigen::Vector3d pc2((pts2d[2](0) - cx) * inv_fx, (pts2d[2](1) - cy) * inv_fy, 1); pc2 /= pc2.norm();

        // calculate distance
        double distances[3];
        const double X0 = pts3d[0](0); const double Y0 = pts3d[0](1); const double Z0 = pts3d[0](2);
        const double X1 = pts3d[1](0); const double Y1 = pts3d[1](1); const double Z1 = pts3d[1](2);
        const double X2 = pts3d[2](0); const double Y2 = pts3d[2](1); const double Z2 = pts3d[2](2);
        ...
        // calculate angles
        double cosines[3];
        const double mu0 = pc0(0); const double mv0 = pc0(1); const double mk0 = pc0(2);
        const double mu1 = pc1(0); const double mv1 = pc1(1); const double mk1 = pc1(2);
        const double mu2 = pc2(0); const double mv2 = pc2(1); const double mk2 = pc2(2);

        cosines[0] = mu1 * mu2 + mv1 * mv2 + mk1 * mk2;
        cosines[1] = mu0 * mu2 + mv0 * mv2 + mk0 * mk2;
        cosines[2] = mu0 * mu1 + mv0 * mv1 + mk0 * mk1;

        // calculate PA, PB, PC with WU Ritt's method
        double lengths[4][3];
        int n = solveForLengths(lengths, distances, cosines);
    }

    // TODO not complete
    int PnPSolver::solveForLengths(double lengths[4][3], const double distances[3], const double cosines[3])
    {
        double p = cosines[0] * 2;
        double q = cosines[1] * 2;
        double r = cosines[2] * 2;

        double inv_d = 1. / (distances[2] * distances[2]);
        double a = inv_d * (distances[0] * distances[0]);
        double b = inv_d * (distances[1] * distances[1]);
    ...
    bool PnPSolver::solvePnPbyDLT(const Eigen::Matrix3d& K, const std::vector< Eigen::Vector3d >& pts3d, const std::vector< Eigen::Vector2d >& pts2d, Eigen::Matrix3d& R, Eigen::Vector3d& t)
    {
        // Check input
    ...
    bool PnPSolver::solvePnPbyEPnP(const Eigen::Matrix3d& K, const std::vector< Eigen::Vector3d >& pts3d, const std::vector< Eigen::Vector2d >& pts2d, Eigen::Matrix3d& R, Eigen::Vector3d& t)
    {
        // Check input data.
    ...
    void PnPSolver::selectControlPoints(const std::vector< Eigen::Vector3d >& pts3d, std::vector< Eigen::Vector3d >& control_points)
    {
        const int n = pts3d.size();
    ...
    void PnPSolver::computeHomogeneousBarycentricCoordinates(const std::vector< Eigen::Vector3d >& pts3d, const std::vector< Eigen::Vector3d >& control_points, std::vector< Eigen::Vector4d >& hb_coordinates)
    {
        const int n = pts3d.size();
    ...
    void PnPSolver::constructM(const Eigen::Matrix3d& K, const std::vector< Eigen::Vector4d >& hb_coordinates, const std::vector< Eigen::Vector2d >& pts2d, Eigen::MatrixXd& M)
    {
    ...
    void PnPSolver::getFourEigenVectors(const Eigen::MatrixXd& M, Eigen::Matrix< double, int(12), int(4) >& eigen_vectors)
    {
    ...
    void PnPSolver::computeL(const Eigen::Matrix< double, int(12), int(4) >& eigen_vectors, Eigen::Matrix< double, int(6), int(10) >& L)
    {
    ...
    void PnPSolver::computeRho(const std::vector< Eigen::Vector3d >& control_points, Eigen::Matrix<double, 6, 1>& rho)
    {
        const int idx0[6] = { 0, 0, 0, 1, 1, 2 };
        const int idx1[6] = { 1, 2, 3, 2, 3, 3 };
        for (int i = 0; i < 6; i++) {
    ...
    void PnPSolver::solveBetaN2(const Eigen::Matrix<double, 12, 4>& eigen_vectors, const Eigen::Matrix< double, int(6), int(10) >& L, const Eigen::Matrix<double, 6, 1>& rho, Eigen::Vector4d& betas)
    {
        //                  [B11 B12 B22 B13 B23 B33 B14 B24 B34 B44]
    ...
    void PnPSolver::solveBetaN3(const Eigen::Matrix<double, 12, 4>& eigen_vectors, const Eigen::Matrix< double, int(6), int(10) >& L, const Eigen::Matrix< double, int(6), int(1) >& rho, Eigen::Vector4d& betas)
    {
        //                  [B11 B12 B22 B13 B23 B33 B14 B24 B34 B44]
    ...
    void PnPSolver::solveBetaN4(const Eigen::Matrix<double, 12, 4>& eigen_vectors, const Eigen::Matrix< double, int(6), int(10) >& L, const Eigen::Matrix< double, int(6), int(1) >& rho, Eigen::Vector4d& betas)
    {
    ...
    void PnPSolver::optimizeBeta(const Eigen::Matrix< double, int(6), int(10) >& L, const Eigen::Matrix< double, int(6), int(1) >& rho, Eigen::Vector4d& betas)
    {
    ...
    void PnPSolver::computeCameraControlPoints(const Eigen::Matrix< double, int(12), int(4) >& eigen_vectors, const Eigen::Vector4d& betas, std::vector< Eigen::Vector3d >& camera_control_points)
    {
    ...
    bool PnPSolver::isGoodBetas(const std::vector<Eigen::Vector3d>& camera_control_points)
    {
        int num_positive = 0;
        int num_negative = 0;
    ...
    void PnPSolver::rebuiltPts3dCamera(const std::vector< Eigen::Vector3d >& camera_control_points, const std::vector< Eigen::Vector4d >& hb_coordinates, std::vector< Eigen::Vector3d >& pts3d_camera)
    {
    ...
    void PnPSolver::computeRt(const std::vector< Eigen::Vector3d >& pts3d_camera, const std::vector< Eigen::Vector3d >& pts3d_world, Eigen::Matrix3d& R, Eigen::Vector3d& t)
    {
    ...
    double PnPSolver::reprojectionError(const Eigen::Matrix3d& K, const std::vector< Eigen::Vector3d >& pts3d_world, const std::vector<Eigen::Vector2d>& pts2d, const Eigen::Matrix3d& R, const Eigen::Vector3d& t)
    {
